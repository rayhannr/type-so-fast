...
## Phase 19 — Backend Migration to Go ✓ Done
...
2. Port read-only routes first (`profile`, `leaderboard`, `display-name`) to validate the AGS Go SDK pattern and header-auth passthrough, plus the admin `client_credentials` token cache (`lib/ags/adminToken.ts` → Go, mutex-guarded since concurrent). ✓ Done — `github.com/AccelByte/accelbyte-go-modular-sdk` (`iam-sdk`, `basic-sdk`, `leaderboard-sdk`, `services-api`) added to `server/go.mod`. `internal/agsconfig` supplies a `repository.ConfigRepository` from env vars plus a `StaticTokenRepository` that just forwards an already-issued access token (the Go service never logs in itself, matching the "no session, forward the header" model from the plan). `internal/ags/{adminToken,profile,leaderboard,displayname}.go` mirror the three `lib/ags/*.ts` files; `internal/handlers/{profile,leaderboard,displayname}.go` mirror the three Next.js routes exactly (same query params, same 401/400 shapes). Public leaderboard reads use a no-op `AuthInfoWriter` to match the TS SDK's unauthenticated call. Verified live against the real AGS dev namespace via `go run .` + curl with a real Device ID token (profile create-or-get, display-name GET with `localName` hint + PATCH persisting, leaderboard alltime/xp/duration/weekly-mode filters, and the 401 path with no auth headers) — not just `go build`. `.github/workflows/deploy-server.yml` now passes `ACCELBYTE_BASE_URL`/`ACCELBYTE_NAMESPACE`/`ACCELBYTE_CLIENT_ID`/`ACCELBYTE_ADMIN_CLIENT_ID`/`ACCELBYTE_ADMIN_CLIENT_SECRET` through as Cloud Run env vars sourced from repo secrets — those 5 secrets have since been added in GitHub.
3. Port the Pusher-touching/stateful routes (`rooms/*`, `match-invites/*`, `friends`, and `pusher/auth` — the highest-risk piece, must use `pusher-http-go`'s own auth helpers so the signature matches what `pusher-js` validates client-side). Verify manually end-to-end with two browser sessions, not just tests — realtime correctness isn't caught by type checks. ✓ Done — `github.com/pusher/pusher-http-go/v5`, `session-sdk`, and `lobby-sdk` added to `server/go.mod`. New `internal/pusherx` wraps `pusher-http-go`'s `Trigger`/`AuthorizePrivateChannel`/`AuthorizePresenceChannel` so the channel-auth signature matches what `pusher-js` validates client-side. `internal/ags/session.go` mirrors `lib/ags/session.ts`'s room + invite-session helpers, including the same optimistic-concurrency `patchSessionWithRetry` (VersionMismatch retry) used by `LockRoom`. `internal/ags/social.go` mirrors `lib/ags/social.ts`'s friends/blocks helpers. New `internal/agserror` carries a passthrough AGS status/errorCode (`*agserror.Detail`) so `JoinRoomByCode`'s invalid/expired/full-session errors keep surfacing the same `errorCode` the frontend's `joinRoomErrorMessage` branches on, instead of collapsing to a generic 500 like the rest of the Go handlers. `internal/handlers/{rooms,matchinvites,friends,blocks,pusherauth}.go` mirror the corresponding Next.js routes. Two SDK defects found and worked around during live verification (not just `go build`): (1) the bulk display-name lookup (`ags.GetUserSummaries`, added in slice 2 but only actually exercised by this slice's room/friends/blocks routes) was calling the public `bulk/basic` endpoint, which 404s in this AGS deployment — switched to the admin `AdminListUserIDByUserIDsV3` endpoint; (2) the generated `GetListOfFriendsShort`/`GetUserIncomingFriendsShort`/`GetIncomingFriendRequestsShort` friends-list methods either route to an admin-only path (403 for a player token) or declare the response as a one-element array when AGS actually returns the plain `{friendIDs, paging}` object (unmarshal error) — `ags.getFriendIDs` calls the self-scoped `/friends/namespaces/{ns}/me` and `/me/incoming` REST paths directly instead. Verified live against the real AGS dev namespace and a real Pusher app via `go run .` + curl with two real Device ID tokens: full room lifecycle (create → generate code → join → get roster/names → progress → start, including the join-by-code 404 with `errorCode` after the room locks), match-invite create/accept/decline, friend request → incoming → accept → list (both directions) → remove, block → list → unblock, and all four `pusher/auth` branches (own `private-user-` channel authorized, someone else's 403, `presence-online-users`, `private-room-*`, and an unrecognized channel 403).
4. Port the remaining CRUD-style routes (achievements, blocks, friends sub-routes, history, matchmaking, progression, pvc/pvp/room-progress, records, session, settings, stats, streak, auth + Google linking). ✓ Done — batched by risk. Cloudsave CRUD batch (history, progression, pvc/pvp/room-progress, records, settings, streak) ✓ Done — `github.com/AccelByte/accelbyte-go-modular-sdk/cloudsave-sdk` added to `server/go.mod`. New `internal/ags/cloudsave.go` mirrors `lib/ags/cloudsave.ts`'s 8 get/save pairs against `PublicPlayerRecordApi`'s single get/put-record-by-key endpoints; `internal/handlers/cloudsave.go` mirrors the 8 corresponding Next.js routes (same body shapes, same fallback defaults: `[]`/`null`/`{}` on a missing key). One SDK defect found and worked around during live verification (not just `go build`): the generated `Value interface{}` field decodes JSON numbers as `json.Number`, not `float64` — a bare `.(float64)` type assertion silently failed and made `records`/`progression.xp`/`pvc.legendWinStreak`/`pvp.winStreak`/`room.winStreak` always look absent; added a `toFloat64` helper that accepts both. Verified live against the real AGS dev namespace via `go run .` + curl with a real Device ID token: all 8 routes' GET/PUT round-trip, plus the 401 path and the fresh-user (never-saved-key) fallback shapes for streak/progression/settings/records/history. Matchmaking batch ✓ Done — `github.com/AccelByte/accelbyte-go-modular-sdk/match2-sdk` added to `server/go.mod` (the TS SDK's `MatchTicketsApi` hits `/match2/v1/...`, a separate service from the legacy `matchmaking-sdk` module already in the dependency tree). New `internal/ags/matchmaking.go` mirrors `lib/ags/matchmaking.ts`'s create/get-status/cancel trio against `match_tickets.{CreateMatchTicket,MatchTicketDetails,DeleteMatchTicket}Short`, same hardcoded `pvp-quick-match` pool; `internal/handlers/matchmaking.go` mirrors the two corresponding Next.js routes. Verified live against the real AGS dev namespace via `go run .` + curl with a real Device ID token: create → get status (matchFound false, isActive true) → cancel → get status again (isActive now false, not a 404 — matches AGS's actual pass-through behavior, not a bug) and the 401 path. Stats batch ✓ Done — `github.com/AccelByte/accelbyte-go-modular-sdk/social-sdk` added to `server/go.mod`. New `internal/ags/statistics.go` mirrors `lib/ags/statistics.ts`'s `submitGameStats`/`getPersonalStats` against `user_statistic.{UpdateUserStatItemValue1,PublicQueryUserStatItems}Short` (the public, player-token-scoped v2 stat-update/v1 bulk-get endpoints — `UserStatisticApi`'s same-named methods are backed by two different classes in the TS SDK, a public one and an admin one, and only the public one works with a player token); same stat-code tables (per-duration/per-mode best-wpm, games-played, total-words-typed, total-xp, level) and the same MAX/INCREMENT update strategies, fanned out concurrently via goroutines instead of `Promise.all`. `internal/handlers/stats.go` mirrors the corresponding Next.js route. Verified live against the real AGS dev namespace via `go run .` + curl with a real Device ID token: GET before any submit (all zeros), POST, GET after (bestWpm/gamesPlayed/totalWordsTyped updated), a second POST with a lower wpm/different mode confirming MAX doesn't regress `bestWpm`/`level` while `gamesPlayed`/`totalWordsTyped`/`total-xp` still increment, and the mode/duration-specific stat codes verified directly against AGS, plus the 401 path. Auth + session batch ✓ Done — `internal/ags/auth.go` mirrors `lib/ags/auth.ts`'s Device ID login/Google login/link/unlink/link-status against IAM's platform-token and account-linking endpoints; `internal/ags/session.go` adds the plain get/set-attributes/leave trio (distinct from the room/invite session helpers already ported in slice 3); `internal/handlers/{auth,session}.go` mirror the five Next.js auth routes and the three `/api/session/:id` routes. Achievements batch ✓ Done — `internal/ags/achievements.go` mirrors `lib/ags/achievements.ts` (catalog fetch + unlock diffing for stat-tied and manual achievements, including the perfectionist/streak/perfect-streak/variety/PvC/PvP/room manual-unlock checks); `internal/handlers/achievements.go` mirrors the three Next.js achievement routes. All three batches verified live against the real AGS dev namespace via `go run .` + curl with a real Device ID token: `/api/auth` device login issuing a real access token, the 401 path on `/api/achievements` and `/api/session/:id` with no auth headers, `/api/achievements/list` catalog fetch, a live `/api/achievements` POST unlocking `perfectionist` on a 100%-accuracy submission, and a full session lifecycle (`/api/rooms` create → `/api/session/:id` GET → PATCH attributes with a round-trip check → DELETE/leave → GET again correctly erroring once left).
5. Delete `app/api/**` once every route has a verified Go counterpart. ✓ Done — before deleting, the `next.config.ts` rewrite proxy itself was verified end-to-end (not just each Go route in isolation): ran the local Go server + `next dev` with `GO_BACKEND_URL` pointed at it, drove the real app in a browser through `localhost:3000` (not curl), and confirmed the leaderboard, achievements catalog, and a full timed round (stat submit, records/history/streak/progression writes, and a live achievement unlock toast) all round-tripped through the proxy with 200s and no console errors. `app/api/**` (all ~35 routes) deleted, along with the server-only modules only they depended on: `lib/ags/{auth,adminToken,sdk,social}.ts`, `lib/api-auth.ts`, `lib/api-error.ts`, `lib/pusher.ts`. The remaining `lib/ags/*.ts` files (`achievements`, `cloudsave`, `displayName`, `leaderboard`, `matchmaking`, `profile`, `session`, `statistics`) were trimmed down to just the type definitions still imported by client-side `lib/queries/*.ts`/components — their AGS-calling functions moved to the Go side and were deleted here. `npx tsc --noEmit` and `npx vitest run` (57 tests) both pass post-deletion.
