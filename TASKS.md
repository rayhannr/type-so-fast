...
## Phase 17 — Friends & Match Invite (in progress)

**T51 — Friends list + direct match invites**
Players become friends by sharing a short AGS-generated `publicId` (Basic service profile field), then invite a friend directly to a PvP match from a Friends screen, bypassing Quick Match's random pairing. Friends can also be removed, blocked, and unblocked; the Friends list shows live online/offline presence. Real-time invite delivery uses Pusher Channels (AGS Lobby's websocket is unusable from a browser, per T50) as the primary path with a 5-minute poll fallback. Full plan, findings, and task checklist in `docs/ags-plans/2026-07-08-friends-match-invite.md` (gitignored, local only, following T50's write-up convention).

Progress so far: AGS backend confirmed (Lobby REST — not the CLI's unrelated `social`/stats service — already active for the namespace and owns friends/block/unblock/presence endpoints; no new Session template needed since `joinability` can be overridden per-call on top of the existing `pvp-quick-match-session`). Pusher account/env vars provisioned, `lib/pusher.ts` server wrapper added. `lib/ags/profile.ts` (get/create/resolve-by-`publicId`) implemented via `@accelbyte/sdk-basic`'s `UserProfileApi` and verified live with a real Device ID player token; `app/api/profile/route.ts` wired into session bootstrap (`useAgsSession`/`AgsSessionContext` now expose `publicId`). Remaining: friends/block REST wrapper, direct-invite session creation, API routes, query hooks, Pusher-driven pending-invite + presence hooks, Friends UI, nav entry, and end-to-end verification.
