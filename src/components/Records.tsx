...
const Records = ({ records, clearRecords }: Props) => {
  ...
  if (records.length === 0) {
    ...
    const midRecord = records[1]
    const maxHeight = 96

    records.forEach((record, index) => {
      switch (index) {
        case 0:
          firstRecord = <Podium record={record} height={maxHeight} color="bg-blue-400" />
          break
        case 1:
          secondRecord = (
            <Podium
              record={record}
              height={record === maxRecord ? maxHeight : ((record * 0.9) / maxRecord) * maxHeight}
              color="bg-blue-300"
            />
          )
          break
        case 2:
          ...
          thirdRecord = (
            <Podium
              record={record}
              height={record === maxRecord ? maxHeight : ((record * param) / maxRecord) * maxHeight}
              color="bg-blue-200"
            />
          )
          break
