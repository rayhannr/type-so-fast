...
export const submitGameStats = async (userId: string, result: GameResultStats): Promise<void> => {
  const statisticApi = UserStatisticApi(sdk)
...
export const getPersonalStats = async (userId: string): Promise<PersonalStats> => {
  const statisticApi = UserStatisticApi(sdk)
