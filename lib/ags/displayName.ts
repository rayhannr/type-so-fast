...
// Headless Device ID accounts are created with an empty IAM uniqueDisplayName (verified live),
...
export const getOrCreateDisplayName = async (accessToken: string, localNameHint?: string): Promise<string> => {
  ...
  if (data.uniqueDisplayName) return data.uniqueDisplayName

  const uniqueDisplayName = localNameHint || generateDisplayName()
  await usersApi.patchUserMe_v3({ uniqueDisplayName })
  return uniqueDisplayName
}

export const updateDisplayName = async (accessToken: string, displayName: string): Promise<string> => {
  const usersApi = UsersApi(createSdk(accessToken))
  await usersApi.patchUserMe_v3({ uniqueDisplayName: displayName })
  return displayName
}
...
export const getUserSummaries = async (userIds: string[]): Promise<UserSummary[]> => {
  ...
  const names = new Map(data.data.map(user => [user.userId, user.uniqueDisplayName]))
