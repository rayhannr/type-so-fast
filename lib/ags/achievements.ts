...
export const unlockStreakAchievementsIfEligible = async (userId: string, accessToken: string, streak: number): Promise<void> => {
...
export const unlockPerfectStreakIfEligible = async (userId: string, accessToken: string, perfectStreak: number): Promise<void> => {
...
export const getAchievementList = async (userId: string, accessToken: string): Promise<AchievementInfo[]> => {
  ...
  const unlockedCodes = new Set(userAchievements.data.filter(item => item.status === UNLOCKED_STATUS).map(item => item.achievementCode))
...
export const diffNewlyUnlocked = async (userId: string, accessToken: string, previousCodes: string[]): Promise<UnlockedAchievement[]> => {
